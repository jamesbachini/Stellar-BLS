...
impl ThresholdBlsDemo {
    pub fn init(env: Env, pubkeys: Vec<BytesN<96>>, dst: Bytes) {
    ...
    pub fn authorize(env: Env, message: BytesN<32>, sigs: Vec<BytesN<192>>) -> Result<(), Err> {
        ...
        for i in 0..pubkeys.len() {
            if i >= sigs.len().try_into().unwrap() { break; }
            let pk_affine = G1Affine::from_bytes(pubkeys.get_unchecked(i.try_into().unwrap()));
            let sig_affine = G2Affine::from_bytes(sigs.get_unchecked(i.try_into().unwrap()));
